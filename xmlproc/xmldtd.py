...
$Id: xmldtd.py,v 2.15 2000/05/11 11:48:24 anonymous Exp $
"""
...
from xmlproc.xmlapp import DTDConsumer
from xmlproc.xmlutils import reg_name, reg_nmtoken, reg_nmtokens
...
class WFCDTD(DTDConsumer):
    ...
    def reset(self):
        ...
        from xmlproc.xmlutils import predef_ents
    ...
    def new_external_entity(self, ent_name, pubid, sysid, ndata):
        if ent_name in self.gen_ents:
            ## FIXME: May warn
            return  # Keep first decl

        if ndata != "" and hasattr(self, "notations"):
            if not ndata in self.notations:
...
class ElementType:
    ...
    def add_attr(self, attr, a_type, a_decl, a_def, parser):
        ...
        elif isinstance(a_type, Tuple) and a_type[0] == "NOTATION":
    ...
    def get_valid_elements(self, state):
        ...
        if self.content_model is None:  # that is, any
    ...
    def get_default_attributes(self):
        ...
        for attr in self.attrhash.values():
            if attr.get_default() is not None:
...
class Attribute:
    ...
    def __init__(self, name, attrtype, decl, default, parser):
        ...
        if default is not None and self.type != "CDATA":
            self.default = "".join(default.split())
        else:
            self.default = default

        # Handling code for special attribute xmlproc:space

        if name == "xmlproc:space":
...
def fnda2fda(transitions, final_state, parser):
    ...
    add_transitions(0, transitions, new_states, start_state, state_key, parser, closure_hash)

    states = list(new_states.keys())
    states.sort()
    ...
    new_states["final"] = pow(2, final_state)
    return new_states
...
def add_transitions(ix, transitions, new_states, cur_state_list, state_key, parser,
    ...
    for (over, destlist) in new_trans.items():
        ...
        if len(destlist) == 1 and destlist[0] in closure_hash:
...
def print_states(states, stop=0):
    ...
    for trans_key in states.keys():
        ...
        print("State: " + repr(trans_key))
        for (to, what) in trans:
            try:
                print("  To: " + repr(to) + " over: " + what)
            except TypeError as e:
                print("ERROR: " + repr(what))
