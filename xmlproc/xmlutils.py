...
from xmlproc import charconv, errors
from xmlproc.xmlapp import ErrorHandler, InputSourceFactory, PubIdResolver, EntityHandler
...
class EntityParser:
    ...
    def set_error_language(self, language):
        ...
        self.errors = errors.get_error_list(language.lower())
        self.err_lang = language.lower()  # only set if supported
    ...
    def parse_resource(self, io: TextIOBase, bufsize: int = 16384):
    ...
    def feed(self, new_data):
        ...
        self.data = (self.data + new_data).replace("\015\012", "\012")
    ...
    def test_reg(self, regexp):
        ...
        return regexp.match(self.data, self.pos) is not None
    ...
    def update_pos(self):
        ...
        breaks = self.data.count("\n", self.last_upd_pos, self.pos)
        self.last_upd_pos = self.pos

        if breaks > 0:
            self.line = self.line + breaks
            self.last_break = self.data.rfind("\n", 0, self.pos)
    ...
    def _push_ent_stack(self, name="None"):
        self.ent_stack.append((self.get_current_sysid(), self.data, self.pos, self.line, self.last_break, self.datasize,
    ...
    def _pop_ent_stack(self):
        (self.current_sysID, self.data, self.pos, self.line, self.last_break,
         self.datasize, self.last_upd_pos, self.block_offset, self.final, dummy) = self.ent_stack[-1]
...
class XMLCommonParser(EntityParser):
    ...
    def parse_xml_decl(self, handler=None):
        """Parses the contents of the XML declaration from after the '<?xmlproc'."""
        ...
        if self.now_at("encoding"):
            ...
            if reg_enc_name.match(enc) is None:
        ...
        if self.now_at("standalone"):
            if textdecl:
                ...
                if reg_std_alone.match(sddecl) is None:
    ...
    def parse_pi(self, handler, report_xml_decl=0):
        ...
        if trgt == "xmlproc":
            ...
            if reg_res_pi.match(trgt) is not None:
                if trgt == "xmlproc:namespace":
                    self.report_error(1003)
                elif trgt != "xmlproc-stylesheet":
    ...
    def _read_char_ref(self):
        ...
        if not (digs == 9 or digs == 10 or digs == 13 or (32 <= digs <= 255)):
            if digs > 255:
...
def matches(regexp, str):
    mo = regexp.match(str)
    return mo is not None and len(mo.group(0)) == len(str)
...
def join_sysids_win32(base, url):
    if len(urlparse(base)[0]) < 2:  # Handles drive identifiers correctly
        if len(urlparse(url)[0]) < 2:
            return os.path.join(os.path.split(base)[0], url)
        else:
            return url
    else:
...
reg_res_pi = re.compile("xmlproc", re.I)
...
ws_trans = str.maketrans("\r\t\n", "   ")  # Whitespace normalization
id_trans = str.maketrans("", "")  # Identity transform
