...
from xmlproc.dtdparser import DTDParser
from xmlproc.xmlapp import Application
from xmlproc.xmldtd import WFCDTD
from xmlproc.xmlutils import XMLCommonParser, EntityParser, OutOfDataException, reg_attval_stop_quote, \
    reg_attval_stop_sing, ws_trans, unhex, reg_hex_digits, reg_digits, reg_ws, join_sysids, reg_int_dtd
...
class XMLProcessor(XMLCommonParser):
    ...
    def parse_att_val(self):
        ...
        while 1:
            ...
            val = val + piece.translate(ws_trans)
    ...
    def parse_literal_entval(self):
        ...
        while 1:
            try:
                ...
                val = val + self.data[self.pos:].translate(ws_trans)
    ...
    def parse_data(self):
        ...
        if end == -1:
            ...
            ampend = self.data.find("&", self.pos, end)
            if ampend != -1:
                end = ampend

        self.pos = end

        if self.data.find("]]>", start, end) != -1:
            self.pos = self.data.find("]]>", start, end)
            self.report_error(3025)
            self.pos = self.pos + 3  # Skipping over it

        if not self.stack:
            res = reg_ws.match(self.data, start)
            if res is None or res.end(0) != end:
    ...
    def parse_doctype(self):
        ...
        if pub_id is not None or sys_id is not None:  # Was there an external id at all?
            if self.read_external_subset:
                try:
                    ...
                    p.parse_resource(join_sysids(self.get_current_sysid(), sys_id))
                finally:
                    p.deref()
                    self.err.set_locator(self)

        if (pub_id is None and sys_id is None) or \
    ...
    def _setup_dtd_parser(self, internal_subset):
        ...
        if self.dtd_listener is not None:
